...
Properties {
    ...
    _lightDirOffset("Light Direction Offset", Vector) = (0, 0, 0, 0)
}
...
SubShader {
    ...
    Pass {
        ...
        void LightDirOffset(inout float3 lightDir)
        {
            lightDir += _lightDirOffset.xyz;
...
//              for(int i=0; i<int(kSamples); i++)
            ...
            OUT.skyColor    = _Exposure * (cIn * getRayleighPhase(lightDir, -eyeRay));
        ...
        // if y < 0 [eyeRay.y > 0] - sky
            ...
            //if(y < 0)
            {
                float3 lightDir = _WorldSpaceLightPos0.xyz;
                LightDirOffset(lightDir);
                col += IN.sunColor * calcSunAttenuation(lightDir, -ray);
            }
        #endif
